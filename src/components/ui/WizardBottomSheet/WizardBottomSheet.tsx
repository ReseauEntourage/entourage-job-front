...
import {
  ...
  StyledPill,
  StyledPillAnchor,
  WIZARD_BOTTOM_SHEET_PILL_HEIGHT,
} from './WizardBottomSheet.styles';

const SWIPE_THRESHOLD = 60;

interface WizardBottomSheetProps {
  children: (mode: 'compact' | 'full') => React.ReactNode;
}

export const WizardBottomSheet = ({ children }: WizardBottomSheetProps) => {
  ...
  useEffect(() => {
    const totalHeight = WIZARD_BOTTOM_SHEET_PILL_HEIGHT + compactHeight;
  ...
  const collapsedOffset = `calc(100% - ${
    WIZARD_BOTTOM_SHEET_PILL_HEIGHT + compactHeight
  }px)`;
