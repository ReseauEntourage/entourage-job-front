...
(function (global, factory) {
  ...
  function translated(el) {
    return (
      Math.abs(uikitUtil.css(el, 'transform').split(',')[4] / el.offsetWidth) ||
      0
    );
  }
  ...
  var Class = {
    connected: function () {
      !uikitUtil.hasClass(this.$el, this.$name) &&
    ...
    methods: {
      toggleElement: function (targets, show, animate) {
        ...
        return uikitUtil.Promise.all(
          uikitUtil.toNodes(targets).map(function (el) {
      ...
      _toggleElement: function (el, show, animate) {
        ...
        if (
          !uikitUtil.trigger(el, 'before' + (show ? 'show' : 'hide'), [this])
        ) {
      ...
      _toggle: function (el, toggled) {
        ...
        if (this.cls) {
          changed =
            uikitUtil.includes(this.cls, ' ') ||
            toggled !== uikitUtil.hasClass(el, this.cls);
          changed &&
            uikitUtil.toggleClass(
              el,
              this.cls,
              uikitUtil.includes(this.cls, ' ') ? undefined : toggled
            );
        } else {
        ...
        uikitUtil.$$('[autofocus]', el).some(function (el) {
          return uikitUtil.isVisible(el) ? el.focus() || true : el.blur();
        });
  ...
  function toggleHeight(ref) {
    ...
    return function (el, show) {
      ...
      var inner = el.hasChildNodes
        ...
        : 0;
      var currentHeight = uikitUtil.isVisible(el)
        ? uikitUtil.height(el) + (inProgress ? 0 : inner)
        : 0;
      ...
      return (
        ...
        return uikitUtil.css(el, initProps);
      });
    };
  }
  ...
  function toggleAnimation(cmp) {
    return function (el, show) {
      ...
      return uikitUtil.Animation.out(
        el,
    ...
    events: [
...

      {
        ...
        handler: function () {
          ...
          if (
            uikitUtil.width(window) - uikitUtil.width(document) &&
            this.overlay
          ) {
          ...
          this.stack &&
            uikitUtil.css(
              this.$el,
              'zIndex',
              uikitUtil.css(this.$el, 'zIndex') + active.length
            );
          ...
          if (this.bgClose) {
            uikitUtil.once(
              this.$el,
              'hide',
              uikitUtil.on(document, uikitUtil.pointerDown, function (ref) {
                var target = ref.target;

                if (
                  uikitUtil.last(active) !== this$1 ||
                  (this$1.overlay && !uikitUtil.within(target, this$1.$el)) ||
                  uikitUtil.within(target, this$1.panel)
                ) {
                  return;
                }
  ...
  function animate(ref) {
    ...
    return function (el, show) {
      return new uikitUtil.Promise(function (resolve, reject) {
        return uikitUtil.once(el, 'show hide', function () {
          el._reject && el._reject();
          el._reject = reject;

          _toggle(el, show);

          var off = uikitUtil.once(
            transitionElement,
            'transitionstart',
            function () {
              uikitUtil.once(
  ...
  function Transitioner(prev, next, dir, ref) {
    ...
    if (show === void 0) show = uikitUtil.noop;
    var props = show(dir);
    ...
    methods: {
      startAutoplay: function () {
        ...
        this.interval = setInterval(function () {
          return (
            (!this$1.draggable || !uikitUtil.$(':focus', this$1.$el)) &&
            (!this$1.pauseOnHover ||
              !uikitUtil.matches(this$1.$el, ':hover')) &&
            !this$1.stack.length &&
            this$1.show('next')
          );
  ...
  var SliderDrag = {
    ...
    methods: {
      start: function () {
        ...
        var off =
          uikitUtil.pointerMove !== 'touchmove'
        ...
        uikitUtil.on(
          document,
          uikitUtil.pointerUp + ' ' + uikitUtil.pointerCancel,
          this.end,
          true
        );
      ...
      move: function (e) {
        ...
        [this.index, this.prevIndex]
          .filter(function (i) {
            return !uikitUtil.includes([nextIndex, prevIndex], i);
          })
          .forEach(function (i) {
  ...
  var SliderNav = {
    ...
    update: {
      write: function () {
        ...
        uikitUtil.toggleClass(
          uikitUtil.$$(this.selNavItem, this.$el).concat(this.nav),
          'uk-hidden',
          !this.maxIndex
        );
  ...
  var Slider = {
    ...
    methods: {
      show: function (index, force) {
        ...
        var prev =
          uikitUtil.hasClass(this.slides, this.clsActive) &&
  ...
  var Slideshow = {
    ...
    computed: {
      animation: function (ref) {
        ...
        return uikitUtil.assign(Animations[animation] || Animations.slide, {
          name: animation,
        });
      },
  ...
  var LightboxPanel = {
    ...
    created: function () {
      ...
      this.items.forEach(function () {
        return uikitUtil.append(list, '<li>');
      });
...

      {
        ...
        handler: function (_, item) {
          ...
          if (
            ...
            uikitUtil.on(video, 'error', function () {
              return this$1.setError(item);
            });
    ...
    methods: {
      ...
      setItem: function (item, content) {
        uikitUtil.trigger(this.$el, 'itemloaded', [
          this,
          uikitUtil.html(this.getSlide(item), content),
        ]);
      },
    ...
    methods: {
      show: function (index) {
        ...
        this.panel =
          ...
          this.$create(
            'lightboxPanel',
            uikitUtil.assign({}, this.$props, { items: items })
          );

        uikitUtil.on(this.panel.$el, 'hidden', function () {
