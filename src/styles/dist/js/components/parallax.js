...
(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined'
    ...
    : typeof define === 'function' && define.amd
      ? define('uikitparallax', ['uikit-util'], factory)
      : ((global =
  ...
  uikitUtil.memoize(function (src) {
  ...
  function getMaxPathLength(el) {
    return Math.ceil(
      Math.max.apply(
        Math,
        [0].concat(
          uikitUtil.$$('[stroke]', el).map(function (stroke) {
            try {
  ...
  var Parallax = {
      ...
      props: function (properties, $el) {
        ...
        return props.reduce(function (props, prop) {
          ...
          if (isColor) {
            ...
            steps = steps.map(function (step) {
              return uikitUtil.toPx(step, attr, this$1$1.$el);
            });

            uikitUtil.css($el, 'background-position-' + prop[2], '');
            bgPos = uikitUtil.css($el, 'backgroundPosition').split(' ')[
              prop[2] === 'x' ? 0 : 1
      ...
      read: function (data) {
        ...
        this.bgProps.forEach(function (prop) {
          ...
          if (span < diff) {
            ...
            var posPercentage =
              dimEl[attr] / uikitUtil.toPx(bgPos, attr, this$1$1.$el);
    ...
    methods: {
      reset: function () {
        ...
        uikitUtil.each(this.getCss(0), function (_, prop) {
          return uikitUtil.css(this$1$1.$el, prop, '');
        });
      },
          ...
          function (css, prop) {
            ...
            switch (prop) {
              ...
              case 'y': {
                ...
                css.transform +=
                  ' translate' +
                  uikitUtil.ucfirst(prop) +
                  '(' +
                  uikitUtil.toFloat(value).toFixed(unit === 'px' ? 0 : 2) +
                  unit +
                  ')';
                break;
              }
  ...
  function parseColor(el, color) {
    return uikitUtil
      .css(uikitUtil.css(el, 'color', color), 'color')
      .split(/[(),]/g)
      .slice(1, -1)
      .concat(1)
      .slice(0, 4)
  ...
  function getUnit(steps) {
    return steps.reduce(function (unit, step) {
      return (
        (uikitUtil.isString(step) && step.replace(/-|\d/g, '').trim()) || unit
      );
    }, '');
  }
  ...
  function covers(el) {
    ...
    var covers =
      uikitUtil.css(
        uikitUtil.css(el, 'backgroundSize', ''),
        'backgroundSize'
      ) === 'cover';
  ...
  var Component = {
    ...
    computed: {
      target: function (ref, $el) {
        ...
        return getOffsetElement(
          (target && uikitUtil.query(target, $el)) || $el
        );
      },
    },
    ...
    update: {
      read: function (ref, types) {
        ...
        var prev = percent;
        percent = ease(
          uikitUtil.scrolledOver(this.target) / (this.viewport || 1),
          this.easing
        );
