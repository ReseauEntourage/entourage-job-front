...
(function (global, factory) {
  ...
  function Transitioner(prev, next, dir, ref) {
    ...
    var to = next
      ...
      : from +
        uikitUtil.dimensions(prev).width * dir * (uikitUtil.isRtl ? -1 : 1);
    ...
    return {
      ...
      translate: function (percent) {
        ...
        uikitUtil.children(list).forEach(function (slide, i) {
      ...
      percent: function () {
        return Math.abs(
          (uikitUtil.css(list, 'transform').split(',')[4] *
            (uikitUtil.isRtl ? -1 : 1) +
            from) /
            (to - from)
        );
      ...
      getItemIn: function (out) {
        ...
        return nextActives[
          uikitUtil.findIndex(nextActives, function (el) {
            return !uikitUtil.includes(actives, el);
          })
        ];
      },
  ...
  function getWidth(list) {
    return uikitUtil.children(list).reduce(function (right, el) {
      return uikitUtil.dimensions(el).width + right;
    }, 0);
  }

  function centerEl(el, list) {
    return (
      uikitUtil.dimensions(list).width / 2 - uikitUtil.dimensions(el).width / 2
    );
  }
  ...
  function getElLeft(el, list) {
    return (
      (el &&
        (uikitUtil.position(el).left +
          (uikitUtil.isRtl
            ? uikitUtil.dimensions(el).width - uikitUtil.dimensions(list).width
            : 0)) *
          (uikitUtil.isRtl ? -1 : 1)) ||
      0
    );
  ...
  var Component = {
    ...
    computed: {
      ...
      sets: function (ref) {
        ...
        var width =
          uikitUtil.dimensions(this.list).width / (this.center ? 2 : 1);
    ...
    connected: function () {
      uikitUtil.toggleClass(
        this.$el,
        this.clsContainer,
        !uikitUtil.$('.' + this.clsContainer, this.$el)
      );
    },
    ...
    update: {
      write: function () {
        ...
        this.navItems.forEach(function (el) {
          ...
          if (index !== false) {
            el.hidden =
              ...
              (this$1$1.sets && !uikitUtil.includes(this$1$1.sets, index));
          }
        });
        ...
        this.slides.forEach(function (slide) {
          return uikitUtil.toggleClass(
            slide,
            this$1$1.clsActive,
            uikitUtil.includes(actives, slide)
          );
        });
        ...
        if (
          this.clsActivated &&
          (!this.sets ||
            uikitUtil.includes(this.sets, uikitUtil.toFloat(this.index)))
        ) {
          this.slides.forEach(function (slide) {
            return uikitUtil.toggleClass(
              slide,
    ...
    methods: {
      reorder: function () {
        ...
        var width =
          uikitUtil.dimensions(this.list).width / 2 -
          uikitUtil.dimensions(next).width / 2;
        var j = 0;
  ...
  function getMaxElWidth(list) {
    return Math.max.apply(
      Math,
      [0].concat(
        uikitUtil.children(list).map(function (el) {
          return uikitUtil.dimensions(el).width;
        })
      )
    );
  }
