...
(function (global, factory) {
  ...
  function Transitioner(prev, next, dir, ref) {
    ...
    return {
      ...
      getActives: function () {
        ...
        return uikitUtil.sortBy(
          slides(list).filter(function (slide) {
            var slideLeft = getElLeft(slide, list);
            return (
              slideLeft >= left &&
              slideLeft + uikitUtil.offset(slide).width <=
                uikitUtil.offset(list).width + left
            );
          }),
          'offsetLeft'
        );
  ...
  function getWidth(list) {
    return slides(list).reduce(function (right, el) {
      return uikitUtil.offset(el).width + right;
    }, 0);
  }

  function getMaxWidth(list) {
    return slides(list).reduce(function (right, el) {
      return Math.max(right, uikitUtil.offset(el).width);
    }, 0);
  }
  ...
  function getElLeft(el, list) {
    return (
      (uikitUtil.position(el).left +
        (uikitUtil.isRtl
          ? uikitUtil.offset(el).width - uikitUtil.offset(list).width
          : 0)) *
      (uikitUtil.isRtl ? -1 : 1)
    );
  }
  ...
  var Component = {
      ...
      finite: function (ref) {
        ...
        return (
          ...
          Math.ceil(getWidth(this.list)) <
            uikitUtil.offset(this.list).width +
              getMaxWidth(this.list) +
              this.center
        );
      },
            ...
            if (slideRight > left) {
              ...
              if (!uikitUtil.includes(sets, i)) {
                ...
                if (
                  this$1.center &&
                  cmp &&
                  slideWidth < leftCenter - uikitUtil.offset(cmp).width / 2
                ) {
    ...
    update: {
      write: function () {
        ...
        uikitUtil
          .$$('[' + this.attrItem + '],[data-' + this.attrItem + ']', this.$el)
          .forEach(function (el) {
            ...
            this$1.maxIndex &&
              uikitUtil.toggleClass(
                el,
                'uk-hidden',
                uikitUtil.isNumeric(index) &&
                  ((this$1.sets &&
                    !uikitUtil.includes(
                      this$1.sets,
                      uikitUtil.toFloat(index)
                    )) ||
    ...
    methods: {
      reorder: function () {
        ...
        var width =
          uikitUtil.offset(this.list).width / 2 -
          uikitUtil.offset(next).width / 2;
        var j = 0;
