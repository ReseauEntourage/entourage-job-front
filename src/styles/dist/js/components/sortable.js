...
(function (global, factory) {
  ...
  function getOffset(element, offset) {
    ...
    if (offset) {
      (assign = uikitUtil.offsetPosition(element)),
        (offsetTop = assign[0]),
        (offsetLeft = assign[1]);
    }
        ...
        getTransitionNodes(target).map(function (child, i) {
          return new uikitUtil.Promise(function (resolve) {
            return setTimeout(function () {
              return uikitUtil.Transition.start(
                child,
                propsOut,
                duration / 2,
                'ease'
  ...
  function slide(action, target, duration) {
    return new uikitUtil.Promise(function (resolve) {
      return requestAnimationFrame(function () {
        ...
        // Find new nodes
        nodes = nodes.concat(
          uikitUtil.children(target).filter(function (el) {
            return !uikitUtil.includes(nodes, el);
          })
        );
        ...
        uikitUtil.Promise.resolve().then(function () {
          ...
          nodes.forEach(function (el, i) {
            return propsFrom[i] && uikitUtil.css(el, propsFrom[i]);
          });
          uikitUtil.css(
            target,
            uikitUtil.assign({ display: 'block' }, targetProps)
          );
          ...
          requestAnimationFrame(function () {
            ...
            uikitUtil.Promise.all(transitions)
              .then(function () {
                nodes.forEach(function (el, i) {
                  return (
                    uikitUtil.parent(el) === target &&
  ...
  function getTransitionProps(target, nodes, currentProps) {
    var propsTo = nodes.map(function (el, i) {
      return uikitUtil.parent(el) && i in currentProps
        ? currentProps[i]
    ...
    var propsFrom = propsTo.map(function (props, i) {
      var from =
        uikitUtil.parent(nodes[i]) === target &&
    ...
    computed: {
      ...
      handles: {
        ...
        watch: function (handles, prev) {
          ...
          uikitUtil.css(handles, {
            touchAction: uikitUtil.hasTouch ? 'none' : '',
            userSelect: 'none',
    ...
    methods: {
      init: function (e) {
        ...
        var ref = this.items.filter(function (el) {
          return uikitUtil.within(target, el);
        });
        ...
        this.origin = uikitUtil.assign(
          { target: target, index: uikitUtil.index(placeholder) },
          this.pos
        );
      ...
      move: function (e) {
        ...
        if (this === sortable) {
          ...
          uikitUtil.trigger(sortable.$el, 'added', [
            sortable,
            this.placeholder,
          ]);
      ...
      remove: function (element) {
        ...
        this.animate(function () {
          return uikitUtil.remove(element);
        });
      },
  ...
  function appendDrag(container, element) {
    ...
    uikitUtil.height(
      ...
      uikitUtil.height(element.firstElementChild)
    );
  ...
  function findTarget(items, point) {
    return items[
      uikitUtil.findIndex(items, function (item) {
        return uikitUtil.pointInRect(point, item.getBoundingClientRect());
      })
    ];
  }
