...
describe('messaging api', () => {
  ...
  describe('postFeedbackRequested (trigger listener)', () => {
    it('marks the selected conversation as not needing feedback on success', async () => {
      ...
      store.dispatch(
        messagingApi.util.upsertQueryData(
          ...
          'conv-1',
          buildConversation({ id: 'conv-1', shouldGiveFeedback: true })
        )
      );
      ...
      expect(
        selectSelectedConversation(store.getState())?.shouldGiveFeedback
      ).toBe(false);
    });
