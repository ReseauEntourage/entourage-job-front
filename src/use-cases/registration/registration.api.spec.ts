...
function selectCreateUserResult(store: ReturnType<typeof createTestStore>) {
  return registrationApi.endpoints.createUser.select(
    CREATE_USER_FIXED_CACHE_KEY
  )(store.getState());
}
...
describe('registration api', () => {
  ...
  describe('createUserRequested (auto-submit listener)', () => {
    it('posts the transformed registration data and succeeds', async () => {
      ...
      expect(selectCreateUserResult(store).isSuccess).toBe(true);
    ...
    it('leaves isLoading true after a success (only the mutation settling resets it)', async () => {
      const store = createTestStore({
        registration: {
          ...slice.getInitialState(),
          selectedFlow: RegistrationFlow.CANDIDATE,
    ...
    it('fails with a generic error without calling the API when there is no registration data', async () => {
      const store = createTestStore({
        registration: {
          ...slice.getInitialState(),
          selectedFlow: RegistrationFlow.CANDIDATE,
      ...
      expect(selectCreateUserResult(store)).toMatchObject({
        isError: true,
        error: 'GENERIC',
      });
    });

    it('fails with DUPLICATE_EMAIL when the API responds with a 409 conflict', async () => {
      const store = createTestStore({
        registration: {
          ...slice.getInitialState(),
          selectedFlow: RegistrationFlow.CANDIDATE,
    ...
    it('fails with a generic error when the API call rejects with a non-conflict error', async () => {
      const store = createTestStore({
        registration: {
          ...slice.getInitialState(),
          selectedFlow: RegistrationFlow.CANDIDATE,
